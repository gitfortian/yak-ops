...
public final class GBase8sDataSourcePlugin extends AbstractJdbcDataSourcePlugin {
  ...
  protected String buildJdbcUrl(String host, int port, String database, JsonNode connectionJson) {
    ...
    if (!hasText(serverName)) {
      throw new DataSourcePluginException(
          Operation.PARAMETER,
          "GBase 8s 使用 host/port/database 生成 JDBC 地址时必须配置 GBASEDBTSERVER");
    }
    if (serverName.indexOf(';') >= 0 || serverName.indexOf(':') >= 0) {
      throw new DataSourcePluginException(
          Operation.PARAMETER, "GBASEDBTSERVER 不能包含 ':' 或 ';'");
    }
    return PREFIX
        + host
        + ":"
  ...
  public boolean acceptsUrl(String jdbcUrl) {
    return jdbcUrl != null
        && jdbcUrl.trim().toLowerCase(Locale.ROOT).startsWith(PREFIX);
  }
  ...
  protected void appendNormalizedFields(JsonNode source, ObjectNode normalized) {
    ...
    if (!hasText(serverName)) {
      throw new DataSourcePluginException(
          Operation.PARAMETER,
          "GBase 8s JDBC 地址或 properties 必须配置 GBASEDBTSERVER");
    }
    normalized.put("serverName", serverName.trim());
    if (!normalized.hasNonNull("schema") || !hasText(normalized.path("schema").asText())) {
  ...
  protected DataSourceCatalog createJdbcCatalog(
    ...
    JdbcConnectionProperties ownerAware =
    ...
    boolean delimitedIdentifiers =
        delimitedIdentifiersEnabled(connection.jdbcUrl(), connection.properties());

    return new GenericJdbcCatalog(ownerAware, connectionTimeoutSeconds, queryTimeoutSeconds) {
      @Override
      protected Connection openConnection() throws Exception {
        return GBase8sDataSourcePlugin.this.openJdbcConnection(
            ownerAware, connectionTimeoutSeconds);
      }
      ...
      protected String quoteIdentifier(String identifier) {
        ...
        if (!simpleIdentifier(value)) {
          throw new IllegalArgumentException(
              "GBase 8s 默认 DELIMIDENT=n，不支持需要双引号的标识符；"
                  + "如需大小写敏感或特殊标识符，请在 properties 中设置 DELIMIDENT=y");
        }
