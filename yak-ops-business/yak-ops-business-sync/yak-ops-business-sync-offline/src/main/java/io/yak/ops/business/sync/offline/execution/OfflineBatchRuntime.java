...
public class OfflineBatchRuntime {
  ...
  public BatchExecution requireLatestOccupyingBatch(Long taskId) { return findLatestOccupyingBatch(taskId).orElseThrow(() -> new IllegalStateException("任务没有运行中的 BatchExecution")); }

  public OfflineJobExecution requireLatestAttempt(BatchExecution batch) {
    Objects.requireNonNull(batch, "BatchExecution 不能为空");
    ExecutionAttempt latest = batch.latestAttempt().orElseThrow(() -> new IllegalStateException("BatchExecution 缺少 Attempt"));
    if (latest.id() == null || latest.id() <= 0L) throw new IllegalStateException("latest Attempt 缺少持久化 ID");
    OfflineJobExecution execution = executionRepository.findById(latest.id()).orElseThrow(() -> new IllegalStateException("latest Attempt persistence 不存在：" + latest.id()));
    if (!Objects.equals(execution.getBatchId(), batch.id())) throw new IllegalStateException("latest Attempt 与 BatchExecution 绑定不一致");
    return execution;
  }
  ...
  public void persistAttempt(OfflineJobExecution execution) {
    ...
    if (execution.getId() == null || execution.getId() <= 0L) throw new IllegalArgumentException("AttemptId 必须大于 0");
    if (!executionRepository.update(execution)) throw new IllegalStateException("更新离线同步 Attempt 失败：" + execution.getId());
  ...
  public void refreshBatch(Long batchId) {
    ...
    BatchExecution batch = batchRepository.findById(batchId).orElseThrow(() -> new IllegalStateException("Attempt 绑定的 BatchExecution 不存在：" + batchId));
  ...
  public OfflineJobExecution cancelWaitingRetry(BatchExecution batch) {
    Objects.requireNonNull(batch, "BatchExecution 不能为空");
    if (batch.status() != BatchStatus.WAITING_RETRY) throw new IllegalStateException("只有 WAITING_RETRY Batch 可以直接取消 Retry 等待");
    OfflineJobExecution latest = requireLatestAttempt(batch);
    if (OfflineExecutionStatus.parse(latest.getStatus()) != OfflineExecutionStatus.FAILED) throw new IllegalStateException("WAITING_RETRY Batch 的 latest Attempt 必须是 FAILED");
    if (!executionRepository.reserveRetry(latest.getId())) throw new IllegalStateException("Retry 已被其他请求保留，无法安全取消等待");
    latest.setNextRetryTime(null); latest.setRetryCreated(true); latest.setUpdateTime(LocalDateTime.now());
    BatchExecution canceled = new BatchExecution(batch.id(), batch.taskId(), batch.batchKey(), batch.trigger(), batch.batchScope(), batch.snapshot(), BatchStatus.CANCELED, batch.attempts());
    if (!batchRepository.update(canceled)) throw new IllegalStateException("取消 WAITING_RETRY Batch 失败：" + batch.id());
    return latest;
  }
  ...
  BatchStatus deriveStatus(OfflineJobExecution latest) {
    ...
    return switch (status) {
      ...
      case FAILED -> latest.getNextRetryTime() == null ? BatchStatus.FAILED : BatchStatus.WAITING_RETRY;
    };
  }
  ...
  private long number(Long value, long fallback) { return value == null ? fallback : value; }
}
