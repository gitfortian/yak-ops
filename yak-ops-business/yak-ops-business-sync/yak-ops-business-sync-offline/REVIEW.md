...
Stage 6 迁移历史只用于理解背景，不得拿历史过渡方案覆盖当前 `DOMAIN.md` contract。
...
- `UNKNOWN` / legacy `LOST` 是否被直接当成 FAILED 自动 Retry；
- V1 同 Task 是否可能同时出现多个 `RUNNING / WAITING_RETRY / UNKNOWN` Batch；
...
- Wave 1 前 `batch_id = NULL` 历史是否重新进入 Retry / Cancel / Reconcile / Task projection；
...
- Wave 1 前 `batch_id = NULL` 历史查询；
- 前端列表、详情和 `last-*` 查询投影；
- 已存在的 PENDING / RUNNING / WAITING_RETRY / UNKNOWN Batch；
- legacy persisted `LOST` 的读取兼容。
...
- Cursor 在失败/不确定状态推进；
- Cursor 回退或跳过未确认区间；
- batchless legacy history 被重新激活；
- Secret 落库、进入快照或日志；
...
- 历史 Stage 6 迁移方案与当前 contract 冲突时，以当前 `REQUIREMENTS.md + DOMAIN.md` 为准。