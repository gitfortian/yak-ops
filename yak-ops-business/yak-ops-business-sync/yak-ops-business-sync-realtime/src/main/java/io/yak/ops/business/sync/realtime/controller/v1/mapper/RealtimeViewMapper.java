...
public class RealtimeViewMapper {
  ...
  public RealtimeViews.Deployment toView(RealtimeJobView.Deployment value) { if (value == null) return null; return new RealtimeViews.Deployment(value.id(), value.sourceDraftRevision(), value.specSummary(), value.artifactDigest(), value.idempotencyKey(), value.engineJobId(), value.runtimeRevision(), toView(value.runtimeEnvironment()), value.status(), value.resultUncertain(), value.errorMessage(), value.createTime(), value.updateTime()); }
