...
public class DataServiceDocumentationController {
  ...
  public Result<ApiDocumentation> documentation(@PathVariable("id") Long id) { return Result.success(reader.get(id)); }

  @Operation(summary = "保存 API 参数与响应文档")
  @PutMapping("/{id}/documentation")
  public Result<ApiDocumentation> saveDocumentation(@PathVariable("id") Long id, @RequestBody DocumentationInput input) {
    if (publisher.managesServiceDefinition(id)) throw new IllegalStateException(
  ...
  public Result<Map<String, Object>> openApi(@PathVariable("id") Long id) { return Result.success(renderer.render(reader.get(id))); }
}
