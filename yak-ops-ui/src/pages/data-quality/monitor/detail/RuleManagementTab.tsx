...
const RuleManagementTab = ({ workspace, running, onRun, onOpenLog, onRefresh, onRemoveMonitor }: RuleManagementTabProps) => {
  ...
  const [draftFilters, setDraftFilters] = useState<RuleFilterState>(createEmptyFilters);
  ...
  const records = useMemo(() => {
    ...
    return monitor.rules.filter((rule) => {
      if (normalizedKeyword && !`${rule.id} ${rule.name} ${rule.templateCode} ${rule.columnName || ''}`.toLowerCase().includes(normalizedKeyword)) return false;
  ...
  const templates = useMemo(() => Array.from(new Map(monitor.rules.map((rule) => [rule.templateCode, { value: rule.templateCode, label: rule.templateCode }])).values()), [monitor.rules]);
  const statusFilter: RuleStatusFilter = filters.enabled === true ? 'ENABLED' : filters.enabled === false ? 'DISABLED' : 'ALL';
  const monitorVisible = useMemo(() => {
    const normalizedKeyword = monitorKeyword.trim().toLowerCase();
    if (!normalizedKeyword) return true;
    return `${monitor.id} ${monitor.name}`.toLowerCase().includes(normalizedKeyword);
  ...
  const advancedSearchContent = (
    <div className="w-[300px]">
      <div className="mb-3 text-[13px] font-semibold text-[#161823]">高级搜索</div>
      <div className="space-y-3">
        <div><div className="mb-1.5 text-xs text-[#667085]">关联范围</div><Select allowClear variant="filled" value={draftFilters.scope} placeholder="全部范围" options={[{ value: 'TABLE', label: '表级' }, { value: 'COLUMN', label: '字段级' }]} onChange={(value) => setDraftFilters((current) => ({ ...current, scope: value }))} className="w-full" /></div>
        <div><div className="mb-1.5 text-xs text-[#667085]">质量维度</div><Select allowClear variant="filled" value={draftFilters.dimension} placeholder="全部维度" options={DIMENSION_ORDER.map((value) => ({ value, label: value }))} onChange={(value) => setDraftFilters((current) => ({ ...current, dimension: value }))} className="w-full" /></div>
      </div>
      <div className="mt-4 flex justify-end gap-2 border-t border-[#f0f1f3] pt-3">
        <YakButton size="small" type="text" className="!text-[#667085]" onClick={reset}>重置</YakButton>
        <YakButton size="small" type="primary" onClick={() => { applyFilters(); setAdvancedOpen(false); }}>应用</YakButton>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex h-full min-h-0 flex-1 items-stretch bg-white">
      <aside className="shrink-0 overflow-hidden bg-[#fbfcfe] px-5 py-5" style={{ width: leftWidth }}>
        <div className="text-[14px] font-semibold text-[#172033]">规则详情</div>
        <div className="mt-4 space-y-1 text-[13px]">
          <div className="flex items-center justify-between rounded-md bg-[#f0f3f8] px-3 py-2 font-medium text-[#27344f]"><span>全部规则</span><span className="rounded-full bg-white px-1.5 text-xs">{stats.ruleCount}</span></div>
          <div className="flex items-center justify-between px-3 py-2 text-[#43506a]"><span>已启用规则</span><span>{stats.enabledRuleCount}</span></div>
      ...
      <main className="min-w-0 flex-1 overflow-auto px-4 py-4">
        <div className="flex min-h-10 items-center justify-between gap-4 border-b border-[#edf0f3] pb-3">
          ...
          <div className="flex shrink-0 items-center gap-2">
            ...
            <Dropdown menu={moreMenu} trigger={['click']}><YakButton type="text" iconOnly aria-label="更多操作" icon={<MoreHorizontal size={15} />} className="!h-8 !w-8 !px-0 !text-[#667085]" /></Dropdown>
          </div>
        </div>
        ...
        <Table<RuleView> rowKey="id" size="small" bordered pagination={false} scroll={{ x: 1280 }} className={dataQualityTableClassName()} dataSource={records} locale={{ emptyText: <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="暂无符合条件的质量规则" /> }} columns={columns} />
        <div className="mt-3 flex justify-end text-xs text-[#8b95a7]">共 {records.length} 条规则</div>
      </main>
    </div>
  );
};
