...
export interface ShellEditorPosition {
  lineNumber: number;
  column: number;
  selectionLength: number;
}

interface ShellMonacoEditorProps {
  id: string;
  value: string;
  ...
  running?: boolean;
  readOnly?: boolean;
  onPositionChange?: (position: ShellEditorPosition) => void;
...
const ShellMonacoEditor = ({
  ...
  useEffect(() => {
    ...
    const uri = monaco.Uri.parse(`inmemory://yak-ops/data-development/shell/${encodeURIComponent(id)}.sh`);
    monaco.editor.getModel(uri)?.dispose();

    const model = monaco.editor.createModel(value, 'shell', uri);
...
export default ShellMonacoEditor;
